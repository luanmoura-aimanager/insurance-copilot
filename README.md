...
- **The origin is an exclusive arc**: `exclusion_id` **or** `coverage_id` — exactly one, enforced by a check constraint. Real foreign keys, so the "cite where this came from" guarantee has referential integrity behind it. `unique (exclusion_id, chunk_index)` and `unique (coverage_id, chunk_index)` make re-indexing idempotent.
...
**`clause_chunk` is not reachable by the SQL worker** — it is absent from `TABLES` in `mcp_servers/postgres_mcp_server.py`, *and* `insurance_ro` has had `SELECT` on it revoked. The omission alone was not protection: `run_query` has no table allowlist, so a `SELECT * FROM clause_chunk` written by the model would pass the text filter and the automatic `LIMIT 100` would dump 100 vectors of 1024 floats into the next call's context. The revoke is surgical — the default privileges for future tables stay in place, and the domain tables stay readable. Similarity search belongs to the RAG worker.
